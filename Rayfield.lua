...
function RayfieldLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Rayfield Gen"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "Rayfield Gen UI"
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			Dropdown.Interact.MouseButton1Click:Connect(function()
				...
				if Dropdown.List.Visible then
					...
					local sb = Dropdown.List:FindFirstChild("SearchBar")
					if sb then
						local tb = sb:FindFirstChildWhichIsA("TextBox")
						if tb then tb.Text = "" end
					end
					for _, opt in ipairs(Dropdown.List:GetChildren()) do
						if opt:IsA("Frame") and opt.Name ~= "Placeholder" and opt.Name ~= "SearchBar" then
							opt.Visible = true
						end
					end
					for _, DropdownOpt in ipairs(Dropdown.List:GetChildren()) do
			...
			-- Search bar injected at top of dropdown list
			local SearchFrame = Instance.new("Frame")
			SearchFrame.Name = "SearchBar"
			SearchFrame.Size = UDim2.new(1, -8, 0, 28)
			SearchFrame.BackgroundColor3 = SelectedTheme.InputBackground or Color3.fromRGB(30, 30, 30)
			SearchFrame.BorderSizePixel = 0
			SearchFrame.LayoutOrder = -1
			do
				local UICorner = Instance.new("UICorner", SearchFrame)
				UICorner.CornerRadius = UDim.new(0, 5)
				local UIStrokeSearch = Instance.new("UIStroke", SearchFrame)
				UIStrokeSearch.Color = SelectedTheme.InputStroke or Color3.fromRGB(65, 65, 65)
			...
			SearchBox.Text = ""
			SearchBox.TextColor3 = SelectedTheme.TextColor or Color3.fromRGB(240, 240, 240)
			SearchBox.PlaceholderColor3 = SelectedTheme.PlaceholderColor or Color3.fromRGB(178, 178, 178)
			...
			SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				for _, opt in ipairs(Dropdown.List:GetChildren()) do
					if opt:IsA("Frame") and opt.Name ~= "Placeholder" and opt.Name ~= "SearchBar" then
						opt.Visible = query == "" or string.find(string.lower(opt.Name), query, 1, true) ~= nil
					end
				end
			end)
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			local function UpdateToggle(Enabled)
				if Enabled then
					...
					TweenService:Create(Toggle.Switch.Indicator.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Color = SelectedTheme.ToggleEnabledStroke}):Play()
					TweenService:Create(Toggle.Switch.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleEnabled, BackgroundTransparency = 0}):Play()
					TweenService:Create(Toggle.Switch.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Color = SelectedTheme.ToggleEnabledOuterStroke}):Play()
				else
					...
					TweenService:Create(Toggle.Switch.Indicator.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Color = SelectedTheme.ToggleDisabledStroke}):Play()
					TweenService:Create(Toggle.Switch.Indicator, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ToggleDisabled, BackgroundTransparency = 0.4}):Play()
					TweenService:Create(Toggle.Switch.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Exponential), {Color = SelectedTheme.ToggleDisabledOuterStroke}):Play()
				end
			end

			UpdateToggle(ToggleSettings.CurrentValue)

			Toggle.MouseEnter:Connect(function()
				TweenService:Create(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {BackgroundColor3 = SelectedTheme.ElementBackgroundHover}):Play()
				TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Thickness = 2}):Play()
			end)

			Toggle.MouseLeave:Connect(function()
				TweenService:Create(Toggle, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Thickness = 1}):Play()
			end)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					warn("Rayfield | Toggle Callback Error:", Response)
		...
		function Tab:CreateSlider(SliderSettings)
			...
			Gradient.Parent = Slider.Main.Progress

			TweenService:Create(Slider, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {BackgroundTransparency = 0}):Play()
			TweenService:Create(Slider.UIStroke, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {Transparency = 0}):Play()
			TweenService:Create(Slider.Title, TweenInfo.new(0.7, Enum.EasingStyle.Exponential), {TextTransparency = 0}):Play()	

			local function UpdateSliderProgress(Value)
				local Percentage = (Value - SliderSettings.Range[1]) / (SliderSettings.Range[2] - SliderSettings.Range[1])
				TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(Percentage, 0, 1, 0)}):Play()
			...
			UpdateSliderProgress(SliderSettings.CurrentValue)

			Slider.MouseEnter:Connect(function()
				TweenService:Create(Slider, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {BackgroundColor3 = SelectedTheme.ElementBackgroundHover}):Play()
				TweenService:Create(Slider.Main.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Thickness = 2}):Play()
			end)

			Slider.MouseLeave:Connect(function()
				TweenService:Create(Slider, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				TweenService:Create(Slider.Main.UIStroke, TweenInfo.new(0.4, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {Thickness = 1}):Play()
			end)

			Slider.Main.Interact.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then 
					SLDragging = true 
					TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {BackgroundTransparency = 0.2}):Play()
				end 
			end)
			...
			Slider.Main.Interact.InputEnded:Connect(function(Input) 
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then 
					SLDragging = false 
					TweenService:Create(Slider.Main.Progress, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
				end 
			end)

			local function GetNewValue(Location)
				local RelativePos = math.clamp((Location - Slider.Main.AbsolutePosition.X) / Slider.Main.AbsoluteSize.X, 0, 1)
				local NewValue = SliderSettings.Range[1] + (RelativePos * (SliderSettings.Range[2] - SliderSettings.Range[1]))
				NewValue = math.floor(NewValue / SliderSettings.Increment + 0.5) * SliderSettings.Increment
				return math.clamp(NewValue, SliderSettings.Range[1], SliderSettings.Range[2])
			end
			...
			UserInputService.InputChanged:Connect(function(Input)
				if SLDragging and (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType == Enum.UserInputType.Touch) then
					local NewValue = GetNewValue(Input.Position.X)
					if NewValue ~= SliderSettings.CurrentValue then
						...
						if not Success then
							warn("Rayfield | Slider Callback Error:", Response)
						end
			...
			Slider.Main.Interact.MouseButton1Down:Connect(function()
				local NewValue = GetNewValue(UserInputService:GetMouseLocation().X)
				SliderSettings.CurrentValue = NewValue
				UpdateSliderProgress(NewValue)
